...
it('renders slow outgoing requests', function () {
    ...
    Pulse::record('slow_outgoing_request', 'GET http://example.com', 1)->max()->count();
    Pulse::record('slow_outgoing_request', 'GET http://example.org', 1)->max()->count();

    // Add entries to the "tail".
    Carbon::setTestNow('2000-01-01 12:00:01');
    Pulse::record('slow_outgoing_request', 'GET http://example.com', 1234)->max()->count();
    Pulse::record('slow_outgoing_request', 'GET http://example.com', 2468)->max()->count();
    Pulse::record('slow_outgoing_request', 'GET http://example.org', 1234)->max()->count();

    // Add entries to the current buckets.
    Carbon::setTestNow('2000-01-01 13:00:00');
    Pulse::record('slow_outgoing_request', 'GET http://example.com', 1000)->max()->count();
    Pulse::record('slow_outgoing_request', 'GET http://example.com', 1000)->max()->count();
    Pulse::record('slow_outgoing_request', 'GET http://example.org', 1000)->max()->count();
