...
<section wire:poll.5s x-data="{
    ...
    @if ($servers->isNotEmpty())
        <div class="grid"
            style="grid-template-columns: max-content minmax(max-content,1fr) max-content minmax(min-content,2fr) max-content minmax(min-content,2fr) minmax(max-content,1fr);">
            <div></div>
            <div></div>
            ...
            @foreach ($servers as $slug => $server)
                ...
                <div wire:key="{{ $slug }}-name"
                    ...
                    <span class="text-base font-bold text-gray-600 dark:text-gray-300"
                        x-bind:title="`Time: {{ number_format($time) }}ms; Run at: ${formatDate('{{ $runAt }}')};`">{{ $server->name }}</span>
                </div>
                <div wire:key="{{ $slug }}-cpu"
                    class="flex items-center {{ $servers->count() > 1 ? 'py-2' : '' }} {{ !$server->recently_reported ? 'opacity-25 animate-pulse' : '' }}">
                    <div class="text-xl font-bold text-gray-700 dark:text-gray-200 w-14 whitespace-nowrap tabular-nums">
                        {{ $server->cpu_current }}%
                    </div>
                </div>
                <div wire:key="{{ $slug }}-cpu-graph"
                    class="flex items-center pr-8 xl:pr-12 {{ $servers->count() > 1 ? 'py-2' : '' }} {{ !$server->recently_reported ? 'opacity-25 animate-pulse' : '' }}">
                    <div wire:ignore class="w-full min-w-[5rem] max-w-xs h-9 relative" x-data="cpuChart({
                        slug: '{{ $slug }}',
                        labels: @js($server->cpu->keys()),
                        data: @js($server->cpu->values()),
                    })">
                        <canvas x-ref="canvas"
                            class="w-full ring-1 ring-gray-900/5 bg-white dark:bg-gray-900 rounded-md shadow-sm"></canvas>
                    </div>
                </div>
                <div wire:key="{{ $slug }}-memory"
                    class="flex items-center {{ $servers->count() > 1 ? 'py-2' : '' }} {{ !$server->recently_reported ? 'opacity-25 animate-pulse' : '' }}">
                ...
                <div wire:key="{{ $slug }}-memory-graph"
                    class="flex items-center pr-8 xl:pr-12 {{ $servers->count() > 1 ? 'py-2' : '' }} {{ !$server->recently_reported ? 'opacity-25 animate-pulse' : '' }}">
                    <div wire:ignore class="w-full min-w-[5rem] max-w-xs h-9 relative" x-data="memoryChart({
                        slug: '{{ $slug }}',
                ...
                <div wire:key="{{ $slug }}-storage"
                    class="flex items-center gap-8 {{ $servers->count() > 1 ? 'py-2' : '' }} {{ !$server->recently_reported ? 'opacity-25 animate-pulse' : '' }}">
                    @foreach ($server->storage as $storage)
                        <div wire:key="{{ $slug . '-storage-' . $storage->directory }}" class="flex items-center gap-4"
                            title="Directory: {{ $storage->directory }}">
                            <div class="whitespace-nowrap tabular-nums">
                                <span
                                    class="text-xl font-bold text-gray-700 dark:text-gray-200">{{ $friendlySize($storage->used) }}</span>
                                <span class="text-sm font-medium text-gray-500 dark:text-gray-400">/
                                    {{ $friendlySize($storage->total) }}</span>
                            </div>

                            <div wire:ignore x-data="storageChart({
                                slug: '{{ $slug }}',
    ...
    <script>
        Alpine.data('cpuChart', (config) => ({
            init() {
                ...
                Livewire.on('servers-chart-update', ({
                    servers
                }) => {
                    if (chart === undefined) {
                        return
                            ...
                            plugins: {
                                ...
                                tooltip: {
                                    ...
                                    callbacks: {
                                        title: () => '',
                                        label: (context) =>
                                            `${context.label} - ${context.formattedValue} MB`
                                    },
                                    displayColors: false,
                                },
                            },
        ...
        Alpine.data('storageChart', (config) => ({
            init() {
                ...
                Livewire.on('servers-chart-update', ({
                    ...
                    const storage = servers[config.slug]?.storage?.find(storage => storage.directory ===
                        config.directory)
