    ...
    >
        ...
        <div class="grid grid-cols-[repeat(4,_auto),_max-content]">
            ...
            @foreach ($servers as $slug => $server)
                ...
                @php
                    $lastUpdated = Carbon\Carbon::parse($lastReading['timestamp'])
                @endphp
                <div class="[&:nth-child(1n+11)]:border-t flex items-center justify-end {{ count($servers) > 1 ? 'py-1' : '' }}"  title="{{ $lastUpdated->fromNow() }}">
                    @if ($lastUpdated->lt(now()->subSeconds(10)))
